...
import { getExchangeRates, getAvailableCoins, getAvailableCombinationsCoins } from "../data/api";
...
export interface ExchangeContextType {
  ...
  addCalculation: (amount: string, fromCurrency: string, toCurrency: string) => void;
...
export const ExchangeProvider: React.FC<ExchangeProviderProps> = ({ children }) => {
  ...
  const addCalculation = async (amount: string, fromCurrency: string, toCurrency: string) => {
    ...
    if (!exchangeData) {
      ...
      showToastError("Erro ao buscar taxas de câmbio. Verifique as moedas e tente novamente.");
      return;
    }
